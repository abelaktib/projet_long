...
class PredictionCallback(tf.keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        disp.plot()
        plt.savefig('figure/learn/confusion_matrix_cnn' +
        ...
        plt.savefig('figure/learn/confusion_matrix_cnn_normalize'
                    + str(self.compteur)+"_" + str(self.lr)+".png")

        print("####################################################")
        with open("confusion_matrix_cnn_" + str(self.lr) + ".csv", "a", encoding="utf-8") as file_m:
            file_m.write(
        ...
        plt.savefig('figure/val/confusion_matrix_cnn_val_normalize'
                    + str(self.compteur)+"_" + str(self.lr)+"cnn.png")

        print("####################################################")
        with open("confusion_matrix_cnn_val" + str(self.lr) + ".csv", "a", encoding="utf-8") as file_t:
            file_t.write(
...
############ COURBE ROC #############œ
        # ROC curve
        fpr, tpr, tresholds = metrics.roc_curve(yval_target_iter.argmax(1), y_vali.argmax(1))
        ...
        # PR Curve
        precision, recall, _ = metrics.precision_recall_curve(yval_target_iter.argmax(1), y_vali.argmax(1))
...
csv_logger = CSVLogger('logcnn.csv', append=True, separator=';')
...
batch_size = 64
EPOCHS = 20
learning_rate_list = [1e-4, 1e-5]  # , 1e-10, 1e-12,1e-15,1e-20
